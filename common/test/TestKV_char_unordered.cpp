...
private:
    ...
    unordered_map<char *, char *, StrHash, StrCompare> m_data;
};
...
void KV::get(char *key, char *&value) {
    pthread_mutex_lock(&m_mutex);
    unordered_map<char *, char *, StrHash, StrCompare>::const_iterator iter = m_data.find(key);
    if (iter == m_data.end()) {
        value = NULL;
    } else {
        value = iter->second;
...
//    static const int LOOP_SIZE = 10 * 1000000; // put time: 11s/10M; get time: 5s/10M; mem: 1.1G/10M
    static const int LOOP_SIZE = 20 * 1000000; // put time: 11s/10M; get time: 4s/10M; mem: 2.1G/20M
