...
private:
    ...
    google::sparse_hash_map<char *, char *, StrHash, eqstr> m_data;
};
...
void KV::get(char *key, char *&value) {
    pthread_mutex_lock(&m_mutex);
    google::sparse_hash_map<char *, char *, StrHash, eqstr>::const_iterator iter = m_data.find(key);
    if (iter == m_data.end()) {
        value = NULL;
    } else {
        value = iter->second;
...
int main(int argc, char *argv[]) {
    ...
    static const int LOOP_SIZE = 1 * 1000000;  // put time: 210s/10M; get time: 60s/10M; mem: 1.1G/10M
